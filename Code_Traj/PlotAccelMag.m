function PlotAccelMag(Time1,Time2,AccelMag1,AccelMag2,...
    classification,titlestr,figstr)
% This routine will plot the AccelMag values from two files for
% comparison purposes
%
...
yvec1=[min(AccelMag1) min(AccelMag2)];
yvec2=[max(AccelMag1) max(AccelMag2)];
ymin1=min(yvec1);
ymax1=max(yvec2);
ymin=ymin1-mod(ymin1,100);
ymax=ymax1-mod(ymax1,100)+100;
diff=AccelMag1-AccelMag2;
mediandiff=median(diff);
% Now plot the AccelMag values
% 
movie_figure1=figure('position',[hor1 vert1 widd lend]);
set(gcf,'MenuBar','none');
set(gca,'Position',[.16 .24 .70 .65]);
plot(Time1,AccelMag1,'b+',Time2,AccelMag2,'r.',Time1,diff,'g');

xlabel('Time-Sec','FontWeight','bold');
ylabel('AccelMag-m/sec^2','FontWeight','bold');
...
hl=legend('ECI AccelMag','ECR AccelMag','Difference','Location','NorthWest');
