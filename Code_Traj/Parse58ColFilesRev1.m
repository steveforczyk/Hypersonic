...
%        : Feb 20,2020 to add addition information to Excel spreadsheet
...
elseif(icase==6)
    ...
    [FileList] = getfilelist(sourcedir,'.asc','True');
    numfiles=length(FileList);
    ExcelFileName='L167aRev4Traj.xls';
    TabName='L167';
...
Results{1,2}='Total #NumObjects';
Results{1,3}='Current Obj #';
Results{1,4}='Header Lines';
Results{1,5}='Current Object Desc';
Results{1,6}='Single File Name';
Results{1,7}='Traj Start Time';
Results{1,8}='Traj End Time';
Results{1,9}='Apogee Time';
Results{1,10}='Apogee-Ht-Km';
...
% Rename some selected Object names
            ...
            Results{nentries+1,3}=nowobj;
            Results{nentries+1,4}=nheader;
            Results{nentries+1,5}=nowObjID;
            singlefilename=strcat(destdir2,prefix,'_',nowObjID,'.asc');
            Results{nentries+1,6}=singlefilename;
            Results{nentries+1,7}=tstart;
            Results{nentries+1,8}=tend;
...
% If the next condition is true this is a data line and write to the new file as is            
        elseif((b2>0) && (tlinelen>1))
            Results{newfiles+1,4}=nheader;
            CVec = textscan(tline,'%f');
            CMat=cell2mat(CVec);
            NowFileTime=CMat(1,1);
            if(NowFileTime>tend)
                tend=NowFileTime;
                Results{nentries+1,8}=tend;
                if(nentries>1)
            ...
            if(NowFileTime<tstart)
                tstart=NowFileTime;
                Results{nentries+1,7}=tstart;
                ab=2;
            end
            NowFileAlt=CMat(3,1)/1000;
            if(NowFileAlt>MaxApogeeKm)
                MaxApogeeKm=NowFileAlt;
                ApogeeTime=NowFileTime;
                Results{nentries+1,9}=ApogeeTime;
                Results{nentries+1,10}=MaxApogeeKm;
            end
            fprintf(fid2,tline);
            fprintf(fid2,'\n');
            ndata=ndata+1;
