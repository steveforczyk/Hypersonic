...
% Created: Feb 23,2019
% Revised: March 10,2019 the Rev1 version calculates 9 items that were
% filled with placeholders before -these are the Topocentric
% Position,Velocity and Accel Values. The calculation of these quantities
% does require the ecef2neu function that is part of the Matlab Aerospace
% toolbox to work. A substitute for that one function is being sought
...
% Calculate the Velocity Components in ENU
        ...
        ViENU(m-1,1)=ENUVel(1,1);
        ViENU(m-1,2)=ENUVel(2,1);
        ViENU(m-1,3)=ENUVel(2,1);
% Calculate hte Accel Components in ENU  
        ENUAccel=RECEFToENU*EFAccel;
        AiENU(m-1,1)=ENUAccel(1,1);
        AiENU(m-1,2)=ENUAccel(2,1);
        AiENU(m-1,3)=ENUAccel(3,1);
...
% Now write each timepoint in ascending order
    ...
    for kk=1:nrows-1
        ...
        if(nowtime>starttime)
            ...
            fprintf(fid,'%16.6f  %12.6f   %12.6f',ECIY(kk,1),ECIZ(kk,1),PiENU(kk,1));%12
            fprintf(fid,' %12.6f   %12.6f  %14.6f',PiENU(kk,2),PiENU(kk,3),VECIX(kk,1));%15
            fprintf(fid,'%14.6f    %14.6f    %14.6f',VECIY(kk,1),VECIZ(kk,1),ViENU(kk,1));%18
            fprintf(fid,'%14.6f    %14.6f',ViENU(kk,2),ViENU(kk,3));%20
