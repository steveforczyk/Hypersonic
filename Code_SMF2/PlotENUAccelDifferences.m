function PlotENUAccelDifferences(titlestr,figstr)
% This routine will plot the asc file ENU acceleration differences
...
ymin1=min(DiffENUAccelx);
ymax1=max(DiffENUAccelx);
ymin2=min(DiffENUAccely);
ymax2=max(DiffENUAccely);
ymin3=min(DiffENUAccelz);
ymax3=max(DiffENUAccelz);
...
plot(ASCTimes,DiffENUAccelx,'b+',ASCTimes,DiffENUAccely,'r.',ASCTimes,DiffENUAccelz,'g');

xlabel('Time-Sec','FontWeight','bold');
ylabel('Accel Differences-mps','FontWeight','bold');
...
hl=legend('ENU Accel X','ENU Accel Y','ENU Accel Z','Location','NorthWest');
