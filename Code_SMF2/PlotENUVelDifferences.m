function PlotENUVelDifferences(titlestr,figstr)
% This routine will plot the asc file ENU velocity differences
...
ymin1=min(DiffENUVelx);
ymax1=max(DiffENUVelx);
ymin2=min(DiffENUVely);
ymax2=max(DiffENUVely);
ymin3=min(DiffENUVelz);
ymax3=max(DiffENUVelz);
...
plot(ASCTimes,DiffENUVelx,'b+',ASCTimes,DiffENUVely,'r.',ASCTimes,DiffENUVelz,'g');

xlabel('Time-Sec','FontWeight','bold');
ylabel('Velocity Differences-meters/sec','FontWeight','bold');
...
hl=legend('ENU VX','ENU VY','ENU VZ','Location','NorthWest');
