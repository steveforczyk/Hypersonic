function PlotTrueVsCalcSR(Time1,Time2,SRTrue,SRCalc,...
    classification,titlestr,figstr)
% This routine will plot the Slant Range True and Calc values
% vs time
...
yvec1=[min(SRTrue) min(SRCalc)];
yvec2=[max(SRTrue) max(SRCalc)];
ymin1=min(yvec1);
ymax1=max(yvec2);
...
% Now plot the Slant Range Values
% 
movie_figure1=figure('position',[hor1 vert1 widd lend]);
set(gcf,'MenuBar','none');
set(gca,'Position',[.16 .24 .70 .65]);
plot(Time1,SRTrue,'b',Time2,SRCalc,'r');

xlabel('Time-Sec','FontWeight','bold');
ylabel('Slant Range-km','FontWeight','bold');
...
hl=legend('SRTrue','SRCalc','Location','NorthWest');
