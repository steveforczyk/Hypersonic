...
% Now get the corresponding ecef position
    ecf=eci2ecf(eci);
    [ENUPos,ENUVel,ENUAccel] = eci2enu(eci,LaunchLat,LaunchLon,LaunchAlt);
...
%     [xEast,yNorth,zUp] = ecef2enu(ecf(1,2),ecf(1,3),ecf(1,4),LaunchLat,LaunchLon,LaunchAlt,spheroid);
    CalcPosENUx(n,1)=ENUPos(1,1);
    CalcPosENUy(n,1)=ENUPos(2,1);
    CalcPosENUz(n,1)=ENUPos(3,1);
    DiffENUx(n,1)=ENUPos(1,1)-PosENUX(n,1);
    DiffENUy(n,1)=ENUPos(2,1)-PosENUY(n,1);
    DiffENUz(n,1)=ENUPos(3,1)-PosENUZ(n,1);
...
%     ENUVel=RECEFToENU*EFVel;
    ...
    ab=2;
% Calculate the Accel Components in ENU  
